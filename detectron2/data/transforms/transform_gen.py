...
from .transform import ExtentTransform, ResizeTransform, OpenCVResizeTransform
...
class OpenCVResizeShortestEdge(TransformGen):
    """
    Scale the shorter edge to the given size, with a limit of `max_size` on the longer edge.
    If `max_size` is reached, then downscale so that the longer edge does not exceed max_size.
    """
    ...
    def get_transform(self, img):
        ...
        scale = size * 1.0 / min(h, w)
        if np.round(scale * max(h, w)) > self.max_size:
            scale = self.max_size * 1.0 / max(h, w)

        return OpenCVResizeTransform(h, w, scale, self.interp)
